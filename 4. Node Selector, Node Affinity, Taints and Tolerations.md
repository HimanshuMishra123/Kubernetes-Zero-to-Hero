...
If no such node with matching label exists or resources are exhausted in the cluster and no autoscaler is involved, the Pod will indeed stay in Pending state forever.

But in case **Karpenter** is present on cluter. That changes things:

* Karpenter looks at the Pod’s scheduling requirements (`nodeSelector`, `affinity`, `taints`, resources, etc.).
* If no existing node matches, Karpenter will **provision a new node** with the labels and requirements needed to satisfy that Pod.
* That’s why your Pod doesn’t stay Pending — Karpenter is launching new nodes that meet the `nodeSelector`.

So the truth is:

✅ Without Karpenter (or Cluster Autoscaler) → Pod stays Pending if no matching node exists.
✅ With Karpenter → Pod triggers **new node creation** that satisfies the `nodeSelector`, so it still gets scheduled.
...
1. **Required During Scheduling, Ignored During Execution:**
   ...
   - Example: Only schedule pods on nodes with the matching kubernetes label disktype=ssd.
